...
public class ItemGunBullet : MonoBehaviour
{
	...
	public void ActivateAll()
	{
		...
		if (bulletHit)
		{
			...
			if (hasHurtCollider)
			{
				gameObject = hurtCollider.gameObject;
			}
	...
	private IEnumerator BulletDestroy()
	{
		...
		while (particleSparks.isPlaying || particleSmoke.isPlaying || particleImpact.isPlaying || hitLight.enabled || shootLine.enabled || (hasHurtCollider && (bool)hurtCollider && hurtCollider.gameObject.activeSelf) || (!hasHurtCollider && (bool)hitGameObject && hitGameObject.activeSelf))
		{
			yield return null;
		}
