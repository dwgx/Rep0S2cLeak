...
public class HurtCollider : MonoBehaviour
{
	...
	public float playerTumbleEnemyHurtDisableTime;
						...
						if ((bool)componentInParent4)
						{
							...
							if (!flag2)
							{
								...
								else if ((bool)componentInParent2)
								{
									if (physReleaseGrab)
									{
										foreach (PhysGrabber item in componentInParent2.playerGrabbing.ToList())
										{
											if (!SemiFunc.IsMultiplayer())
											{
												item.ReleaseObject(componentInParent2.photonView.ViewID, 0.25f);
												continue;
											}
											item.photonView.RPC("ReleaseObjectRPC", RpcTarget.All, false, 0.25f, componentInParent2.photonView.ViewID);
										}
									}
									if (PhysObjectHurt(componentInParent2, physImpact, physHitForce, physHitTorque, apply: true, destroyLaunch: false))
									{
										flag2 = true;
									}
								}
		...
		else
		{
			...
			else
			{
				...
				if (playerTumbleTime > 0f && _player.playerHealth.health > 0)
				{
					...
					if (playerTumbleEnemyHurtDisableTime > 0f)
					{
						_player.tumble.OverrideEnemyHurt(playerTumbleEnemyHurtDisableTime);
					}
					flag2 = true;
					flag = true;
				}
		...
		if (hitForce > 0f)
		{
			if (hitForce >= 5f && physGrabObject.playerGrabbing.Count > 0 && !physGrabObject.overrideKnockOutOfGrabDisable)
			{
				foreach (PhysGrabber item in physGrabObject.playerGrabbing.ToList())
				{
					...
					item.photonView.RPC("ReleaseObjectRPC", RpcTarget.All, false, 1f, -1);
				}
			}
			Vector3 center = Collider.bounds.center;
